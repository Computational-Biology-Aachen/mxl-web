...
<script lang="ts">
  ...
  function buildFitConfig():
    ...
    // Showcase-only experiment (not wired into the shared $lib/Fit.svelte):
    // a trained NN block picks the "lm" backend's analytic-Jacobian path
    // (jacobian_wrapper.c's lmder, `FitInitRequest.jacobianWat`'s doc
    // comment) instead of unconditionally forcing "adjoint" — viable here
    // because this showcase's block is small (4×2, ~20 fit params), where
    // one augmented forward solve per Jacobian evaluation beats both
    // fit_wrapper.c's finite differences (numerically unreliable at this
    // scale/`scale` factor — confirmed by direct testing: it fails
    // immediately, zero real steps) and adjoint's Adam (far slower
    // per-step convergence for a problem this small). A model.ts with a
    // much larger block should still force "adjoint", same as before —
    // this experiment doesn't change that path.
    const backend: FitBackend | undefined = undefined;
    let jacobianWat: string | undefined;
    if (nnBlockFitIdx.length > 0) {
      try {
...
<div class="fit-panel">
  ...
  {#if hasTrainedNNBlock}
    {#if mode === "ensemble"}
      <p class="nn-note">
        Also training {trainedBlockKeys.length} NN block(s), this fit uses the analytic-Jacobian
        LM backend. Each ensemble member trains from its own independently Glorot-initialized

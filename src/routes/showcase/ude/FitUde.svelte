<!--
 @component
 UDE showcase fork of $lib/Fit.svelte, fit a model's parameters to uploaded
...
<script lang="ts">
  ...
  const UDE_SHOWCASE_CSV: ParsedCsv = {
  ...
  let csv = $state<ParsedCsv | null>(UDE_SHOWCASE_CSV);
  ...
  // empirically: this showcase's own default Alpha+Beta+NN-block fit
  // throws "Maximum call stack size exceeded" under "lm-jacobian" (caught
  // by buildFitConfig's own try/catch, so it fails as a visible error
  // rather than crashing, but it's still a bad default) while "adjoint"
  // handles the exact same fit fine. Root cause is presumably
  ...
  // empirically: this showcase's own default 25-parameter fit
  ...
  // room) — raising it to 60 (> the 25 fit params here) made both converge

...
<script lang="ts">
  ...
  } from "./fit/curriculumRunner";
  import CurriculumStagesEditor from "./fit/CurriculumStagesEditor.svelte";
  import {
    buildFitConfig,
    residualPerPoint,
    type BackendChoice,
  ...
  import LineChart, { type PhaseRegion } from "./LineChart.svelte";
  import {
    mulberry32,
    sampleDistribution,
  ...
  // suggested one per model (below), not forced. (BackendChoice itself now
  // lives in ./fit/fitConfig.ts, shared with FitNde.svelte/FitUde.svelte.)
  ...
  // FitTargetEntry/FitConfig/buildFitConfig now live in ./fit/fitConfig.ts,
  // shared with FitNde.svelte/FitUde.svelte and every curriculum stage
  // (./fit/curriculumRunner.ts).

  // ---- Curriculum schedule (optional; a single 100%-cutoff stage
  // reproduces a plain fit exactly) — see ./fit/curriculum.ts. The single
  ...
  // ./fit/curriculum.ts's finalStageBudget / ./fit/curriculumRunner.ts).
